...
private:
	...
	void StrIns(const char *s, int pos, int len);
public:
	MyString(int max = 1);
	...
	int ThereIs(const char *s, int start = 0)const;
	void CharIns(char);//默认插在最后
	void CharIns(char, int pos);
	void CharDel(int pos);
	void StrIns(const char *s, int pos);
	void StrIns(MyString &s, int pos);
	void StrDel(int pos, int len);
	void StrRpl(const MyString &s, const MyString &t);
	void StrRpl(const MyString &s, const char *t);
	void StrRpl(const char *s, const MyString &t);
	void StrRpl(const char *s, const char *t);
...
inline void MyString::StrIns(const char * s, int pos, int len)
{
	int i;
	if (pos > length)
		pos = length;
...
inline int MyString::ThereIs(const char * s, int start) const
{
	int *next = new int[StrLen(s)];
	GetNext(s, next);
	int v = KMP(s, start, next);
	delete[] next;
	return v;
}
...
inline void MyString::StrIns(const char * s, int pos)
{
	StrIns(s, pos, StrLen(s));
	return;
}

inline void MyString::StrIns(MyString & s, int pos)
{
	StrIns(s.data, pos, s.length);
	return;
}
...
{
	StrRpl(s.data, t.data);
}

inline void MyString::StrRpl(const MyString & s, const char * t)
{
	StrRpl(s.data, t);
}

inline void MyString::StrRpl(const char * s, const MyString & t)
{
	StrRpl(s, t.data);
}

inline void MyString::StrRpl(const char * s, const char * t)
{
	int pos = ThereIs(s);
	while (pos != -1) {
		StrDel(pos, StrLen(s));
